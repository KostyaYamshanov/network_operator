...
SimpleSolution::SimpleSolution(const SimpleConfig& config)
    : config_(config), int_bits_(16), frac_bits_(16), num_params_(2) {
...
SimpleConfig SimpleSolution::get_config()
{
    return config_;
}

void SimpleSolution::decode(const std::vector<int>& chromosome_params,
...
std::unique_ptr<ISolution> SimpleSolution::clone() const {
    return std::make_unique<SimpleSolution>(config_);
}

std::vector<float> SimpleSolution::getParameters() const {
...
void SimpleSolution::greyToVector(const std::vector<int>& grey_code) {
    try {
        if (grey_code.empty()) {
